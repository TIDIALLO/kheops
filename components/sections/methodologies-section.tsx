...
import
{
  CalendarRange, TrendingUp, Layers3, FileSpreadsheet, ChevronDown, ChevronUp,
  Calendar, BarChart2, Database, Table, FileSpreadsheetIcon, Bug, ClipboardList, GitBranch
} from 'lucide-react'
            ...
            >
              ...
              {item.items && (
                <div className="space-y-3">
                  {item.items.map((example: string, idx: number) => (
...
{
  const [openMethod, setOpenMethod] = useState<string | null>('planning')
  const [openTechnique, setOpenTechnique] = useState<string | null>('planning')
